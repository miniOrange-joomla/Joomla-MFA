...
class MoUtility
{
	...
	{
		return preg_match(MoOTPConstants::PATTERN_PHONE,MoUtility::processPhoneNumber($phone),$matches);
	}

	public static function isCountryCodeAppended($phone)
	{
		return preg_match(MoOTPConstants::PATTERN_COUNTRY_CODE,$phone,$matches) ? true : false;
	}

	public static function processPhoneNumber($phone)
	{
		$phone = preg_replace(MoOTPConstants::PATTERN_SPACES_HYPEN,"", ltrim(trim($phone),'0'));
	...
	{
		$option = Factory::getApplication()->input->get->post(['option']);
		return current_user_can('manage_options') 
				&& MoUtility::micr() 
				&& isset($option)
				&& 'mo_customer_validation_settings'==Factory::getApplication()->input->get->post(['option']);
	}
	...
	public static function checkIfValidSubmission()
	{
		...
		|| !check_admin_referer( MoOTPConstants::FORM_NONCE )) ? FALSE : TRUE;
	}
}